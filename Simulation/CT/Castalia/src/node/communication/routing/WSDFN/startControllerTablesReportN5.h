...
void WSDFN::startTables(int n)
{
	...
	if(this->isController){
		...
		timerTable[0].t0=0.89; //inicial time  almost 9 seconds
		timerTable[0].time=1.0; //repeat in 2 minutes
		...
		timerTable[1].t0=1+dataTime; //inicial time 39 seconds (9 +30 seconds)
		timerTable[1].time=1.0; //repeat in 2 minutes
		...
		//Timer of sersor with sequencial delay
			timerTable[2].t0=1+((0.0+spaceDelay)*atoi(SELF_NETWORK_ADDRESS)/1000);
...
//		timerTable[2].t0=1.0+ ( (double)interval / 1000 ); //inicial time 9 seconds + 0 a 100 miliseconds for avoid collision
		...
		timerTable[2].time=1.0; //repeat in 2 minutes
...
//		scheduleTimer(1);
		...
		for(int i=0;i<lastNode; i++){
			...
			timerTableC[0][i].t0=0.89; //inicial time  almost 9 seconds
			timerTableC[0][i].time=1.0; //repeat in 2 minutes
			...
			timerTableC[1][i].t0=1+dataTime; //inicial time 39 seconds (9 +30 seconds)
			timerTableC[1][i].time=1.0; //repeat in 2 minutes
...
//Timer of sersor with sequencial delay
			timerTableC[2][i].t0=1+((0.0+spaceDelay)*i/1000);
			#ifdef DEBUGFLOW
...
//			timerTableC[2][i].t0=9.0; //inicial time 9 seconds
			...
			timerTableC[2][i].time=1.0; //repeat in 2 minutes
			...
			if(n==5) actionTableC[3][i].value1.valueInt16_1=0;
