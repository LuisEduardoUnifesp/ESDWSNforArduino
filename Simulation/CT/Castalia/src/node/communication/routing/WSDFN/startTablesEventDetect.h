...
#ifndef _START_TABLES_EVENT_H_
#define _START_TABLES_EVENT_H_

#include "Structs.h"
#include <string.h>

/**
 * Initializes tables so that network works as Event Detector
 *
 */
void WSDFN::startTables()
{
	...
	//timer 0: 2 minutes for preparate the detect event temperature in all sensors
	...
	if(!(this->isController)){
		...
		//timer 2: 2 minutes for read temperature in all sensors
...
//		flowTable[0].priority=1;
		...
		flowTable[0].fa.sourceMask=0xFF; //source address mask 1bit
		flowTable[0].fa.destinationAddress=0;
		flowTable[0].fa.destinationMask=0;
...
//		flowTable[1].priority=1;
		...
		flowTable[1].fa.destinationMask=0xFF;
...
//		flowTable[0].priority=1;
		...
		flowTable[0].fa.destinationMask=0xFF;
		for(int j=1; j< SIZEOF_FLOW_TABLE;j++) flowTable[j].active = false;

		//action 2=Display Sensor
		actionTable[2].actionID=2;
