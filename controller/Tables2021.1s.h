...
void startTables(int n)
{
  ...
  double interval;
  int spaceDelay = 150; //time space between sensors send data (in miliseconds)
  int dataTime = 900;  //duration of report data sensors (in miliseconds)
  int repeat = 1000; // time for repetitions
  unsigned long t0 = timezero + 183500ul; //TX_CONTROL start time 5min3s
  unsigned long transmission = 200ul; //2seconds after
  int lastNode = n;
  //    #ifdef DEBUGFLOW
